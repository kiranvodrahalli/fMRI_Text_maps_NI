...
def run_20dim_SRMICA_all(tr_times, tst_times):
    W_test = {}
    for mask in mask_types:
        W, S, test_data = run_20dim_SRMICA(mask, tr_times, tst_times)
        W_test[mask] = (W, S, test_data)
    return W_test

def run_20dim_SRMICA(mask_type, tr_times, tst_times):
    dataset = '20dim-SRMICA-' + mask_type
    nfeature = 20 # 20dim
    niter = 10 # enough
    randseed = 0
    fmat = 'NPZ'
    ...
    # rondo options
    output_path = '/jukebox/norman/knv/thesis/sherlock/RESULTS/SRMICA_fMRI/' + mask_type + '/'
    input_path = '/jukebox/norman/knv/thesis/sherlock/fMRI_masked/'
    ...
    if fmat == 'NPZ':  # If the data is in .npz
        ...
        if fmat == 'NPZ':
            ...
            np.savez_compressed(output_path + 'WsS_tr_tst.npz',
...
if __name__ == '__main__':
    run_20dim_SRMICA_all(xrange(1973/2+1), xrange(1973+1, 1973))
