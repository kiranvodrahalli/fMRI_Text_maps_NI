...
from linear_maps import learn_linear_maps, add_prev_time_steps, keep_only_first_time_step
...
# MODIFY TO RUN OVER ARBITRARY SRM SIZE
...
# ndims = SRM/SRMICA dimension
def experiment_50_50_split(ndims, avg_variant, date_of_wordvecs, subtract_temporal_average_bool):
	if avg_variant == 'avg':
...
# ndims for SRM and SRMICA is the dimension
def past_timesteps_experiment_50_50_split(ndims, improper_classification, num_previous, avg_variant, date_of_wordvecs, subtract_temporal_average_bool):
	if avg_variant == 'avg':
		...
		for mask in srms.keys():
			...
			avg_srm_prev_times = add_prev_time_steps(avg_srm_tst_data, num_previous)
	...
	for mask in fmris.keys():
		...
		# Truncate after learning linear maps btwn long and long
			...
			TF_prediction = keep_only_first_time_step(TF_prediction, num_previous)
			TF_prediction_flipped = keep_only_first_time_step(TF_prediction_flipped, num_previous)

			# evaluate in text space
			FT_prediction = keep_only_first_time_step(FT_prediction, num_previous)
			FT_prediction_flipped = keep_only_first_time_step(FT_prediction_flipped, num_previous)
...
if __name__ == "__main__":
	...
	results_dict = {} # improper_classification = None
	avg_options = ['srm', 'srmica', 'pca'] # leave out avg
	...
	dimensions = [20, 50, 80, 100]

	# load most recent
	# results_dict = p.load(open(PATH_TO_RESULTS + 'RESULTS/main_results_dict_top1_Mar4_multidimfMRI_SINGLE_SIDED_NEUROIMAGE.p', 'rb'))

	# modify this to iterate over different SRM dimensions as well
	# make PCA consistent with SRM dimension
	# already handled all of SRM
	# need to fix srmICA files that have been loaded; run later after we update the results dict with PCA.
	for avg in ['srm']: #avg_options:
		for dim in [1000]:
			for weight in ['weighted']: #weight_options:
				for subtract in ['subtract']: #subtract_options:
					key_tuple = (avg, weight, subtract, dim, 0)
					print "====================================================="
					print key_tuple
					results_dict[key_tuple] = experiment_50_50_split(dim, avg, 'sep12' + weight, (subtract is 'subtract'))
					# save after each iteration
					p.dump(results_dict, open(PATH_TO_RESULTS + 'RESULTS/main_results_dict_top1_Mar6_1000dimfMRI_SINGLE_SIDED_NEUROIMAGE.p', 'wb'))
					if avg != 'avg':
						...
						for num_previous in num_previous_timesteps: # all
							key_tuple = (avg, weight, subtract, dim, num_previous)
							print key_tuple
							results_dict[key_tuple] = past_timesteps_experiment_50_50_split(dim, None, num_previous, avg, 'sep12' +  weight, (subtract is 'subtract'))
							p.dump(results_dict, open(PATH_TO_RESULTS + 'RESULTS/main_results_dict_top1_Mar6_1000dimfMRI_SINGLE_SIDED_NEUROIMAGE.p', 'wb'))
							print "//////////////////////"
					print "====================================================="
